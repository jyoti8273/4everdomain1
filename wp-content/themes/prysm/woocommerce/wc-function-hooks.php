...
function bixol_cwoocommerce_remove_function() {
...
add_action( 'init', 'bixol_cwoocommerce_remove_function' );

/* Product Category */
add_action( 'woocommerce_before_shop_loop', 'bixol_woocommerce_nav_top', 2 );
function bixol_woocommerce_nav_top() { ?>
...
add_filter( 'woocommerce_after_shop_loop_item', 'bixol_woocommerce_product' );
function bixol_woocommerce_product() {
	global $product;
	?>
			...
			<div class="woocommerce-product-meta">
				...
				<?php } else { ?>
					<div class="woocommerce-add-to-cart">
				    	...
				    	<span class="view-tooltip tooltip-add-cart"><?php echo esc_html__('Add To Cart', 'bixol'); ?></span>
				    	<span class="view-tooltip tooltip-view-cart"><?php echo esc_html__('View cart', 'bixol'); ?></span>
					</div>
				<?php } ?>
				<?php if (class_exists('YITH_WCWL')) { ?>
					<div class="woocommerce-wishlist">
						<span class="view-tooltip"><?php echo esc_html__('Add To Wishlist', 'bixol'); ?></span>
				...
				<?php if (class_exists('YITH_WCQV')) { ?>
					<div class="woocommerce-quick-view">
						<span class="view-tooltip"><?php echo esc_html__('Quick View', 'bixol'); ?></span>
...
function bixol_hide_page_title()
{
    return false;
}
add_filter('woocommerce_show_page_title', 'bixol_hide_page_title');

/* Replace text Onsale */
add_filter('woocommerce_sale_flash', 'bixol_custom_sale_text', 10, 3);
function bixol_custom_sale_text($text, $post, $_product)
{
...
function bixol_loop_shop_per_page(){
	$product_per_page = '12';
...
add_filter( 'loop_shop_per_page', 'bixol_loop_shop_per_page' );
...
function bixol_woocommerce_sg_social_share() {?>
		<div class="woocommerce-social-share">
		    ...
		    <a class="pin-social" title="<?php echo esc_attr__('Pinterest', 'bixol'); ?>" target="_blank" href="//pinterest.com/pin/create/button/?url=<?php echo esc_url(the_post_thumbnail_url( 'full' )); ?>&media=&description=<?php the_title(); ?>"><i class="zmdi zmdi-pinterest"></i></a>
		</div>
<?php }

/* Product Single: Gallery */
add_action( 'woocommerce_before_single_product_summary', 'bixol_woocommerce_single_gallery_start', 0 );
function bixol_woocommerce_single_gallery_start() { ?>
	<?php echo '<div class="woocommerce-gallery col-xl-6 col-lg-6 col-md-6">'; ?>
<?php }
add_action( 'woocommerce_before_single_product_summary', 'bixol_woocommerce_single_gallery_end', 30 );
function bixol_woocommerce_single_gallery_end() { ?>
	<?php echo '</div><div class="col-xl-6 col-lg-6 col-md-6">'; ?>
<?php }

/* Rating */
function bixol_rating($rating_html, $rating) {
	global $product;
	$rating_count = $product->get_rating_count();
	if($rating_count == 0) {
		$rating_count = esc_html__( 'No', 'bixol' );
	}
...
add_filter( 'woocommerce_product_get_rating_html', 'bixol_rating', 10, 2);

/* Ajax update cart item */
add_filter('woocommerce_add_to_cart_fragments', 'bixol_woo_mini_cart_item_fragment');
function bixol_woo_mini_cart_item_fragment( $fragments ) {
	global $woocommerce;
    ob_start();
    ?>
    <div class="widget_shopping_cart">
    	<div class="widget_shopping_title">
    		<?php echo esc_html__( 'Shopping Cart', 'bixol' ); ?> <span class="widget_cart_counter">(<?php echo sprintf (_n( '%d item', '%d items', WC()->cart->cart_contents_count, 'bixol' ), WC()->cart->cart_contents_count ); ?>)</span>
    	</div>
			...
			<?php if ( ! WC()->cart->is_empty() ) : ?>
								...
								<div class="cart-product-meta">
									...
									<?php
										echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
											...
											esc_attr__( 'Remove this item', 'bixol' ),
											esc_attr( $product_id ),
				...
				<?php else : ?>
					...
					<li class="empty">
						...
						<span><?php esc_html_e( 'Your cart is empty', 'bixol' ); ?></span>
						<a class="btn" href="<?php echo get_permalink( wc_get_page_id( 'shop' ) ); ?>"><?php echo esc_html__('Browse Shop', 'bixol'); ?></a>
					</li>
        ...
        <?php if ( ! WC()->cart->is_empty() ) : ?>
			<div class="widget_shopping_cart_footer">
				<p class="total"><strong><?php esc_html_e( 'Subtotal', 'bixol' ); ?>:</strong> <?php echo WC()->cart->get_cart_subtotal(); ?></p>

				<?php do_action( 'woocommerce_widget_shopping_cart_before_buttons' ); ?>

				<p class="buttons">
					<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="btn wc-forward"><?php esc_html_e( 'View Cart', 'bixol' ); ?></a>
					<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="btn checkout wc-forward"><?php esc_html_e( 'Checkout', 'bixol' ); ?></a>
				</p>
			</div>
		<?php endif; ?>
    </div>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'bixol_woocommerce_sidebar_cart_count_number' );
function bixol_woocommerce_sidebar_cart_count_number( $fragments ) {
	ob_start();
	?>
	<span class="widget_cart_counter">(<?php echo sprintf (_n( '%d', '%d', WC()->cart->cart_contents_count, 'bixol' ), WC()->cart->cart_contents_count ); ?>)</span>
	<?php
...
add_filter( 'woocommerce_add_to_cart_fragments', 'bixol_woocommerce_sidebar_cart_count_number_header' );
function bixol_woocommerce_sidebar_cart_count_number_header( $fragments ) {
	ob_start();
	?>
...
add_filter( 'woocommerce_output_related_products_args', 'bixol_related_products_args', 20 );
  function bixol_related_products_args( $args ) {
...
/* Pagination Args */
function bixol_filter_woocommerce_pagination_args( $array ) { 
	$array['end_size'] = 1;
	$array['mid_size'] = 1;
    return $array; 
}; 
add_filter( 'woocommerce_pagination_args', 'bixol_filter_woocommerce_pagination_args', 10, 1 ); 