<?php
	...
	if(!empty($prysm['blog-background'])){
	$breadcrumb_bg = $prysm['blog-background']['url'] ? $prysm['blog-background']['url'] : ""; }
	else{ $breadcrumb_bg = get_template_directory_uri(). "";}
	$top_zero = has_nav_menu( 'header_menu' ) ? '' : 'top-zero';
?>
<!-- Start of Breadcrumb section
	============================================= -->
	<section id="pr-breadcrumb" class="pr-breadcrumb-section position-relative <?php print esc_attr($top_zero); ?>" data-background="<?php echo esc_url($breadcrumb_bg);?>">
		<span class="background_overlay"></span>
		<div class="container">
			<div class="pr-breadcrumb-content  text-center headline">
				<?php if(!empty($prysm['Blog_heading'])) : ?>
					<h2><?php echo esc_html($prysm['Blog_heading'] ? $prysm['Blog_heading'] : 'Blog');?> </h2>
				<?php else : ?>
					<h2><?php echo esc_html__('Blog', 'prysm'); ?></h2>
				<?php endif; ?>
				<?php if(!empty($prysm['Blog_breadcrumb'])): ?>
...
<!-- Start of blog feed section
	...
	============================================= -->
<?php
	if(!empty($prysm['blog-contactsection'])):
	if($prysm['blog-contactsection']['Blog_bottom_section']):
	if(!empty($prysm['footer_top_area'])):
?>
	<!-- Start of newslatter section
	============================================= -->
	<section id="pr-newslatter" class="pr-newslatter-section position-relative">
		<span class="pr-newslatter-img position-absolute"><img src="<?php echo esc_url($prysm['blog-contactsection']['blog-main-bglogo']['url']);?>"></span>
		<div class="container">
			<div class="pr-newslatter-content d-flex justify-content-between align-items-center">
				<div class="pr-newslatter-text headline pera-content">
					<p><?php echo wp_kses($prysm['blog-contactsection']['blog-bottom-title'] , prysm_expanded_alowed_tags())?></p>
				</div>
				<?php if($prysm['blog-contactsection']['Blog_bottom_form']): ?>
